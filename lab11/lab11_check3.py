...
# -*- coding: utf-8 -*-
"""
Created on Tue Nov 19 12:34:57 2019
...
class BallDraw(object):
    def __init__(self, parent):
        ...
        self.ball_x, self.ball_y = random.randint(10, 390), random.randint(10,390)
        
        self.ball_radius = random.randint(5,10)
        self.ball_dx, self.ball_dy = random.randint(-8,8), random.randint(-8,8)  # the movement of the ball object
        self.ball_color = random.choice(colorList)

        self.b = Ball(self.ball_x, self.ball_y, self.ball_dx, self.ball_dy, self.ball_radius, self.ball_color)
        ...
        self.maxx = 500  # canvas width, in pixels
        self.maxy = 500  # canvas height, in pixels
...
#        self.canvas.create_oval(self.b.bounding_box(), fill=self.b.get_color())
...
#        self.canvas.after(self.wait_time)
    ...
    def animate(self):
        ...
        while (self.b.some_inside(self.maxx, self.maxy) == True):
            # Move the ball
            self.canvas.delete("all")
            ...
            # Pause execution.  This allows the eye to catch up
            self.canvas.after(self.wait_time)
            if (self.b.check_and_reverse(self.maxx, self.maxy) == True):
                self.b.move()
            else:
                self.b.move()
